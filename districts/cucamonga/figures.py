...
# 		self.date_range = date_range
	...
	def mpl_water_quality(self,constituent,unit,start_date=None,end_date=None):
		...
		for well in df['dms_site_id'].unique():
			...
			ax.plot(df_well['date'],df_well['result'])
			ax.scatter(df_well['date'],df_well['result'],label=well)
	...
	def extractions(self):
		...
		with st.expander('Data'):
			st.dataframe(
				df,
				column_order=['dms_site_id','date','production_af','changelog'],
				hide_index=True,
		...
		for date in df['start_date'].unique():
			bottom = 0
			# st.markdown(df['date'].iloc[0])
			# st.markdown(date)
			# st.markdown(type(date))
